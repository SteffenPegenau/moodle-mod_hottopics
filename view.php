...
// Post question
	$anonynmous_post = $hotquestion->anonymouspost;
...
$questions = $DB->get_records_sql("SELECT q.id, q.hotquestion, q.userid, q.time, q.anonymous , CAST(q.content AS varchar(max)) as content, count(v.voter) as votecount
                              FROM {$CFG->prefix}hotquestion_questions q
                              LEFT JOIN {$CFG->prefix}hotquestion_votes v
                              ON v.question = q.id
                              WHERE q.hotquestion = $hotquestion->id
                                    AND q.time >= {$current_round->starttime}
                                    AND q.time <= {$current_round->endtime}
                              GROUP BY  q.id, q.hotquestion, CAST(q.content AS varchar(max)), q.userid, q.time, q.anonymous 
                              ORDER BY votecount DESC, q.time DESC");
